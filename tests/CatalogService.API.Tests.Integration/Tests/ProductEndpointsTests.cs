...
public class ProductEndpointsTests(CustomWebApplicationFactory factory) : BaseEndpointTests(factory), IClassFixture<CustomWebApplicationFactory>
{
    ...
    public async Task GetProduct_ReturnsProduct()
    {
        ...
        await _client.PostAsJsonAsync("/api/catalog/products", product);
    ...
    {
        // Arrange
        await _client.PostAsJsonAsync("/api/catalog/products", new Product { Id = 1, Name = "Test-1" });
        await _client.PostAsJsonAsync("/api/catalog/products", new Product { Id = 2, Name = "Test-2" });
    ...
    public async Task AddProduct_ReturnsCreated()
    {
        ...
        var items = await _client.GetFromJsonAsync<List<Product>>("/api/catalog/products");
    ...
    public async Task UpdateProduct_ReturnsOk()
    {
        ...
        await _client.PostAsJsonAsync("/api/catalog/products", entiity);

        // Act
        entiity.Name = "Updated";
        var response = await _client.PutAsJsonAsync($"/api/catalog/products/{productId}", entiity);
        ...
        var product = await _client.GetFromJsonAsync<Product>($"/api/catalog/products/{productId}");
    ...
    public async Task DeleteProduct_ReturnsOk()
    {
        // Arrange
        var productId = 1;
        await _client.PostAsJsonAsync("/api/catalog/products", new Product { Id = productId, Name = "Test-Delete" });

        // Act
        var response = await _client.DeleteAsync($"/api/catalog/products/{productId}");
